...
static Value PartitionConstants(const IcebergScanPlanBindData &bind, const IcebergScanTask &task) {
	...
	auto spec = metadata.partition_specs.find(task.manifest_file.partition_spec_id);
	if (spec == metadata.partition_specs.end()) {
		throw InvalidConfigurationException("'partition_spec_id' %d doesn't exist in the metadata",
		                                    task.manifest_file.partition_spec_id);
	}
	...
	for (idx_t i = 0; i < bind.partition_source_ids.size(); i++) {
		...
		do {
			...
			for (const auto &partition : task.data_file.entry.data_file.partition_info) {
				...
				if (bind.transaction_entries.count(&task.data_file.entry)) {
...
static void IcebergScanPlanFunction(ClientContext &context, TableFunctionInput &data, DataChunk &output) {
	...
	for (; count < STANDARD_VECTOR_SIZE; count++, state.file_id++) {
		...
		auto &file = task->data_file.entry.data_file;
		...
		output.data[7].SetValue(count, PartitionConstants(bind, *task));
