...
IcebergScanReader::IcebergScanReader() {
...
IcebergEqualityDeleteFastFilter::BuildResult IcebergScanReaderGlobalState::GetOrCreateEqualityDeleteFastFilter(
...
IcebergEqualityDeleteReadColumn IcebergScanReader::AddEqualityDeleteColumn(
...
vector<IcebergEqualityDeleteReadColumn> IcebergScanReader::AddEqualityDeleteColumns(
...
void IcebergScanReader::ApplyPartitionConstants(const unordered_map<int32_t, Value> &constants,
...
ReaderInitializeType IcebergScanReader::InitializeReader(
    MultiFileReader &multi_file_reader, MultiFileReaderData &reader_data, const MultiFileBindData &bind_data,
	...
	multi_file_reader.MultiFileReader::FinalizeBind(reader_data, bind_data.file_options, bind_data.reader_bind,
	                                                scan_columns, scan_column_ids, context,
	...
	return multi_file_reader.CreateMapping(context, reader_data, scan_columns, scan_column_ids, table_filters,
	                                       gstate.file_list, bind_data.reader_bind, bind_data.virtual_columns);
}

unique_ptr<Expression> IcebergScanReader::CreateEqualityDeleteExpression(
    const vector<reference<const IcebergEqualityDeleteFile>> &delete_files,
...
void IcebergScanReader::FinalizeChunk(MultiFileReader &multi_file_reader, ClientContext &context,
                                      const MultiFileBindData &bind_data, BaseFileReader &reader,
	...
	auto &iceberg_state = global_state->Cast<IcebergScanReaderGlobalState>();

	//! Base class finalization first
	multi_file_reader.MultiFileReader::FinalizeChunk(context, bind_data, reader, reader_data, input_chunk, output_chunk,
	                                                 executor, global_state);
...
MultiFileReaderVirtualColumnBinding IcebergScanReader::GetVirtualColumnExpression(
    MultiFileReader &multi_file_reader, ClientContext &context, MultiFileReaderData &reader_data,
    const vector<MultiFileColumnDefinition> &local_columns, const idx_t column_id, const LogicalType &type,
	...
	return multi_file_reader.MultiFileReader::GetVirtualColumnExpression(context, reader_data, local_columns, column_id,
	                                                                     type, local_idx);
}

void IcebergScanReader::BindOptions(MultiFileReader &multi_file_reader, MultiFileOptions &options, MultiFileList &files,
	...
	multi_file_reader.MultiFileReader::BindOptions(options, files, return_types, names, bind_data);
}
