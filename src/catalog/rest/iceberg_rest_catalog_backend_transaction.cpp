...
bool IcebergRESTCatalogBackend::CanCommitMultipleTables(const IcebergTransactionAlterUpdate &alter_update) const {
	if (!SupportsMultiTableCommit()) {
		return false;
	}
...
void IcebergRESTCatalogBackend::CleanupMetadataFiles(ClientContext &context, const vector<string> &paths) {
	if (!attach_options.remove_files_on_delete || paths.empty()) {
		return;
	}
...
void IcebergRESTCatalogBackend::RefreshRetryTables(IcebergTransactionAlterUpdate &alter_update,
...
bool IcebergRESTCatalogBackend::CommitStateUnknown(const ErrorData &error) const {
...
void IcebergRESTCatalogBackend::DoMultiTableCommitUpdates(IcebergTransactionAlterUpdate &alter_update,
                                                          ClientContext &context) {
	...
	for (idx_t attempt = 0;; attempt++) {
		auto transaction_info = StageMultiTableCommit(*context.db, alter_update, context);
		...
		auto result = IRCAPI::CommitMultiTableUpdate(context, *this, transaction_json);
		if (result.Success()) {
			return;
		}
		if (!CommitIsRetryable(transaction_info.retryable, transaction_info.retry_config.num_retries, result,
		                       attempt)) {
			result.Throw(GetBaseUrl().GetURLEncoded());
		}
...
void IcebergRESTCatalogBackend::DoSingleTableCommitUpdates(IcebergTransactionAlterUpdate &alter_update,
                                                           ClientContext &context) {
	D_ASSERT(supported_urls.count("POST /v1/{prefix}/namespaces/{namespace}/tables/{table}"));
	...
	std::optional<IcebergRetryBackoff> backoff;
	for (idx_t attempt = 0;; attempt++) {
		auto table_transaction_info = StageSingleTableCommit(*context.db, *table_info, context);
		if (!backoff) {
		...
		auto result =
		    IRCAPI::CommitTableUpdate(context, *this, identifier._namespace.value, identifier.name, transaction_json);
		if (result.Success()) {
			return;
		}
...
bool IcebergRESTCatalogBackend::SupportsMultiTableCommit() const {
	return !attach_options.disable_multi_table_commit && supported_urls.count("POST /v1/{prefix}/transactions/commit");
}

void IcebergRESTCatalogBackend::CommitTableUpdates(ClientContext &context, IcebergTransactionAlterUpdate &update) {
	if (CanCommitMultipleTables(update)) {
		DoMultiTableCommitUpdates(update, context);
	} else {
		DoSingleTableCommitUpdates(update, context);
	}
}

void IcebergRESTCatalogBackend::CreateSchema(ClientContext &context, const vector<string> &name) {
	rest_api_objects::CreateNamespaceRequest request;
	request._namespace.value = name;
	request.properties = case_insensitive_map_t<string>();
	IRCAPI::CommitNamespaceCreate(context, *this, RESTObjectToJSONString(request));
}

void IcebergRESTCatalogBackend::DropSchema(ClientContext &context, const vector<string> &name) {
	IRCAPI::CommitNamespaceDrop(context, *this, name);
}

void IcebergRESTCatalogBackend::UpdateSchemaProperties(ClientContext &context, const vector<string> &name,
                                                       const case_insensitive_map_t<string> &updates,
                                                       const set<string> &removals) {
	rest_api_objects::UpdateNamespacePropertiesRequest request;
	request.updates = updates;
	request.removals = vector<string>(removals.begin(), removals.end());
	IRCAPI::CommitNamespacePropertiesUpdate(context, *this, RESTObjectToJSONString(request), name);
}

void IcebergRESTCatalogBackend::DropTable(ClientContext &context, const IcebergTable &table) {
	IRCAPI::CommitTableDelete(context, *this, table.schema.namespace_items, table.name);
}

void IcebergRESTCatalogBackend::RenameTable(ClientContext &context, const IcebergTable &table, const string &new_name) {
	rest_api_objects::RenameTableRequest request;
	request.source._namespace.value = table.schema.namespace_items;
	request.source.name = table.name;
	request.destination._namespace.value = table.schema.namespace_items;
	request.destination.name = new_name;
	IRCAPI::CommitTableRename(context, *this, RESTObjectToJSONString(request));
}
